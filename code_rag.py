...
ЗАЧЕМ ЭТО (и чем отличается от "просто спросить ИИ с большим контекстом"):

  Любая современная LLM с большим окном контекста и так может ответить
  "где в коде обработчик клика по карточке?", если запихнуть ей всю
  кодовую базу целиком. Смысл этого проекта — не в том, чтобы обойти
  эту возможность, а в том, чтобы на практике собрать пайплайн
  локализации кода: индекс СИМВОЛОВ (а не произвольных кусков текста),
  гибридный поиск по ним и один финальный запрос к локальной модели,
  который лишь ПОДТВЕРЖДАЕТ и ОБЪЯСНЯЕТ находку, а не выдумывает номера
  строк. Опыт интеграции ИИ в свою программу — с честным пайплайном
  retrieval → grounding → generation, без "модель угадала путь к файлу".

АРХИТЕКТУРА:

  1. Индексация — не построчная нарезка, а РАЗБОР СИМВОЛОВ. По каждому
     файлу регулярными выражениями (под особенности языка) находятся:
       - функции / методы / классы (def, function, fn, func, class)
       - обработчики: Express/Flask-роуты (app.get(...)), JS
         addEventListener, JSX-пропсы onClick={...}, инлайн-атрибуты
         onclick="..." в HTML
       - CSS-селекторы (#id, .class { ... })
     Для каждого найденного символа фиксируются: имя, тип, файл,
     точные start_line/end_line (границы блока вычисляются подсчётом
     фигурных скобок или по отступам — для Python) и сам код блока.
     Это даёт то, чего построчные чанки дать не могут: гарантированно
     верные границы "обработчик начинается и заканчивается здесь".

  2. Retrieval — гибридный поиск по символам:
       - BM25 (лексика: точные совпадения имён, включая разбиение
         camelCase/snake_case на смысловые части)
       - Векторные эмбеддинги (ChromaDB, многоязычная модель) — чтобы
         русский вопрос находил англоязычные имена по смыслу
     Оба списка сливаются через Reciprocal Rank Fusion (RRF).

  3. Generation — ОДИН запрос к локальной LLM (Ollama) на весь набор
     кандидатов: модель в строгом JSON решает, какой из них РЕАЛЬНО
     отвечает на вопрос, и оценивает уверенность (0-100). Путь к файлу
     и номера строк в итоговом ответе ВСЕГДА берутся из Python-данных
     по номеру кандидата — модель не может их "придумать" или исказить,
     она только выбирает и коротко объясняет.
